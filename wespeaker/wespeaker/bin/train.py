...
from wespeaker.utils.utils import get_logger, parse_config_or_kwargs, set_seed, \
    spk2id
import wespeaker.models
print(dir(wespeaker.models))
...
def train(config='conf/config.yaml', **kwargs):
    ...
    if rank == 0:
        try:
            ...
            if checkpoint is None:
                ...
                # exit(1) # Continue for test restart convenience
    ...
    writer = None
    if rank == 0:
        tb_dir = configs.get('tensorboard_dir', os.path.join(configs['exp_dir'], 'tensorboard'))
    ...
    utt2lang = None
    lang2id = None
    train_lang_label = configs.get('train_lang_label', None)
    if train_lang_label is not None:
        utt2lang = {}
        with open(train_lang_label, 'r') as f:
            for line in f:
                parts = line.strip().split()
                if len(parts) >= 2:
                    utt_id, lang_id = parts[0], int(parts[1])
                    utt2lang[utt_id] = lang_id
        # lang2id 已經在 utt2lang 中使用數字 ID，直接建立反向映射
        lang2id = {v: v for v in set(utt2lang.values())}  # identity mapping
        if rank == 0:
            logger.info("Language adversarial training: {} languages, {} utterances".format(
                len(lang2id), len(utt2lang)))
    ...
    if configs.get('model_init', None) is not None:
    ...
    if rank == 0:
        ...
        for line in pformat(model).split('\n'):
            logger.info(line)
        # !!!IMPORTANT!!!
        # 移除 jit.script 匯出，transformers 類別不支援 scripting
    ...
    if checkpoint is not None:
        loaded_checkpoint = load_checkpoint(model, checkpoint)
    ...
    from wespeaker.utils.lr_utils import build_param_groups
    
    discriminative_lr_config = configs.get('discriminative_lr', {})
    if discriminative_lr_config.get('enabled', False):
        if rank == 0:
            logger.info("Enabling Discriminative Learning Rates")
        
        lr_groups = discriminative_lr_config.get('lr_groups', {})
        base_lr = configs['scheduler_args']['initial_lr']
        
        # 使用 build_param_groups 分組參數
        param_groups = build_param_groups(model_to_run, base_lr, lr_groups)
    ...
    if 'scale_ratio' not in configs['scheduler_args']:
    ...
    # 1/19 scaler = torch.cuda.amp.GradScaler(enabled=configs['enable_amp'])
    # If bfloat16 is enabled, we generally don't need GradScaler as the dynamic range is larger
    use_bfloat16 = configs.get('bfloat16', False)
    scaler = torch.amp.GradScaler('cuda', enabled=(configs['enable_amp'] and not use_bfloat16))
    if rank == 0 and use_bfloat16:
        logger.info("Using bfloat16 for training (GradScaler disabled)")
    
    # Check if auto analysis is enabled in config
    # Use .get() with default True or False based on your preference
    enable_auto_analysis = configs.get('enable_auto_analysis', False)
    ...
    for epoch in range(start_epoch, configs['num_epochs'] + 1):
        ...
        if rank == 0: # Check rank
            if epoch % configs['save_epoch_interval'] == 0 or epoch > configs[
                    'num_epochs'] - configs['num_avg']:
                save_checkpoint(
                    model, os.path.join(model_dir,
                                        'model_{}.pt'.format(epoch)), optimizer, scheduler)

                # Auto Analysis (Sequential Mode - Blocking)
                if enable_auto_analysis:
                    try:
                        script_path = os.path.abspath("auto_analyze.sh")
                        if os.path.exists(script_path):
                            logger.info("Triggering auto analysis for epoch {} (Sequential/Blocking)".format(epoch))
                            # Using subprocess.run to pause training until analysis completes.
                            subprocess.run(['bash', script_path, str(epoch), configs['exp_dir']], check=False)
                    except Exception as e:
                        logger.warning("Failed to trigger auto analysis: {}".format(e))
                else:
                    logger.info("Auto analysis disabled in config, skipping for epoch {}".format(epoch))
