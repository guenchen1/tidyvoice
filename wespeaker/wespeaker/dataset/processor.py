...
def custom_load(file_obj_or_path):
    try:
        # Try using soundfile which is successfully installed and working
        # soundfile.read supports file paths and file-like objects (bytes)
        # FORCE float32 to avoid Double vs Float mismatch in Resample/Conv1d
        ...
        # Fallback to torchaudio if soundfile fails (unlikely for wav)
        return torchaudio.load(file_obj_or_path)
            ...
            with stream.extractfile(tarinfo) as file_obj:
                try:
                    if postfix in ['spk']:
                        ...
                        # Direct fix for torchaudio/torchcodec issue
                        # Use custom_load which prefers soundfile
                        waveform, sample_rate = custom_load(file_obj)
...
def parse_raw(data):
    ...
    def read_audio(wav):
        if wav.endswith('|'):
            ...
            data = p.stdout.read()
            waveform, sample_rate = custom_load(io.BytesIO(data))
        else:
            waveform, sample_rate = custom_load(wav)
        return waveform, sample_rate
...
def lang_to_id(data, utt2lang, lang2id=None):
    """ 將語言標籤加入 sample (用於語言對抗訓練)

        Args:
            data: Iterable[{key, wav/feat, label, ...}]
            utt2lang: Dict[str, int] - utterance ID 直接對應到語言 ID (整數)
            lang2id: (未使用，保留為相容性)

        Returns:
            Iterable[{key, wav/feat, label, lang_label, ...}]
    """
    for sample in data:
        assert 'key' in sample
        key = sample['key']
        if key in utt2lang:
            sample['lang_label'] = utt2lang[key]  # 直接使用整數 ID
        else:
            sample['lang_label'] = -1  # 無語言標籤
        yield sample
...
def speed_perturb(data, num_spks):
    ...
    """
    speeds = [1.0, 0.9, 1.1]
    resamplers = {} # Cache resamplers to avoid re-computing kernels
    ...
    for sample in data:
        ...
        if speed_idx > 0:
            ...
            # Cache the resampler for performance (avoids Sinc kernel calc every time)
            # Use tuple key (orig, new)
            resampler_key = (sample_rate, new_freq)
            ...
            # Apply resampling
            wav = resamplers[resampler_key](waveform)
