...
def Dataset(data_type,
    ...
    """ Construct dataset from arguments
        ...
        Args:
            ...
            utt2lang: Dict[str, str] - 可選，utterance ID 到語言代碼映射
            lang2id: Dict[str, int] - 可選，語言代碼到數字 ID 映射
    """
    assert data_type in ['shard', 'raw', 'feat']
    frontend_type = configs.get('frontend', 'fbank')
    frontend_args = frontend_type + "_args"
    if frontend_type == 'raw_fbank':
        # fallback: raw_fbank 也用 fbank_args
        frontend_args = 'fbank_args'
    ...
    # 語言標籤 (用於語言對抗訓練)
    if utt2lang is not None and lang2id is not None:
        dataset = Processor(dataset, processor.lang_to_id, utt2lang, lang2id)
    ...
    if data_type == 'feat':
        ...
        if frontend_type == 'fbank':
            dataset = Processor(dataset, processor.compute_fbank,
                                **configs['fbank_args'])
        elif frontend_type == 'raw_fbank': # 進入你的自定義融合模式
            # 同樣調用 compute_fbank，但開啟 return_raw_wav 開關
            dataset = Processor(dataset, processor.compute_fbank,
                                return_raw_wav=True,
                                **configs['fbank_args'])
