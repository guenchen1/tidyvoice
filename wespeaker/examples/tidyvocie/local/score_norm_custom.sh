...
echo "Using cohort set: $cohort_set"
echo "Using score norm method: $score_norm_method"
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
  ...
  for x in $trials; do
    python wespeaker/bin/score_norm.py \
      ...
      --eval_emb_scp ${exp_dir}/embeddings/${eval_dataset}/xvector.scp \
...
if [ $stage -le 3 ] && [ $stop_stage -ge 3 ]; then
  ...
  for x in ${trials}; do
    ...
    python wespeaker/bin/compute_metrics.py \
      ...
      2>&1 | tee -a ${scores_dir}/${eval_dataset}_${score_norm_method}${top_n}_result
