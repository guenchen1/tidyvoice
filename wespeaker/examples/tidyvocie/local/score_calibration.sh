...
#		2025 Aref Farhadipour - University of Zurich  (areffarhadi@gmail.com, aref.farhadipour@uzh.ch)
#         	
...
# Use tidyvoice_dev for calibration (trials directory)
trials_dir=${data}/tidyvoice_dev/trials
# Use tidyvoice_dev as the calibration dataset
calibration_set=tidyvoice_dev

if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
  echo "Score calibration set"
  # Compute duration for calibration set (tidyvoice_dev)
  if [ ! -f ${data}/${calibration_set}/utt2dur ]; then
    python tools/wav2dur.py ${data}/${calibration_set}/wav.scp ${data}/${calibration_set}/utt2dur > ${data}/${calibration_set}/dur.log
  fi
  # generate trial for calibration using tidyvoice_dev
  if [ ! -e ${trials_dir}/${calibration_trial} ]; then
    python tools/generate_calibration_trial.py --utt2dur ${data}/${calibration_set}/utt2dur --trial_path ${trials_dir}/${calibration_trial}
  fi

  python wespeaker/bin/score.py \
    --exp_dir ${exp_dir} \
    --eval_scp_path ${exp_dir}/embeddings/${calibration_set}/xvector.scp \
    --cal_mean True \
    --cal_mean_dir ${exp_dir}/embeddings/${calibration_set} \
  ...
  python wespeaker/bin/score_norm.py \
    ...
    --eval_emb_scp ${exp_dir}/embeddings/${calibration_set}/xvector.scp \
    --mean_vec_path ${exp_dir}/embeddings/${calibration_set}/mean_vec.npy
fi

if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
  echo "Prepare calibration factors"
  # gather calibration factor using tidyvoice_dev
  mkdir -p ${exp_dir}/scores/calibration
  cat ${data}/${calibration_set}/utt2dur > ${exp_dir}/scores/calibration/utt2dur
